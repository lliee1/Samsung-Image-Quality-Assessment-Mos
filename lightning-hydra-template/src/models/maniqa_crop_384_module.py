...
class Maniqa_crop_384Module(LightningModule):
    ...
    def model_step(
        ...
        """
        img, score = batch["I"], batch["score"].squeeze()
        batch_size = img.size(0)
        num_patch = img.size(1)
        img = img.view(-1, img.size(2), img.size(3), img.size(4))

        # (b*n)
        pred = self.forward(img)
        pred = pred.view(batch_size, num_patch)
        pred = pred.mean(1)
    ...
    def test_model_step(
        ...
        img, img_name = batch["I"], batch["img_name"]
        batch_size = img.size(0)
        num_patch = img.size(1)
        img = img.view(-1, img.size(2), img.size(3), img.size(4))
    ...
    def test_step(
        ...
        :param batch_idx: The index of the current batch.
        """
        pred, img_name = self.test_model_step(batch)
        ...
        # loss, pred, score = self.model_step(batch[0])
        # path = batch[1]
        # if loss.item() > 0.01:
        #     with open("/root/dacon/data/loss.txt", "a") as f:
        #         f.write(
        #             str(loss)
        #             + "\t\t\t\t"
        #             + str(path)
