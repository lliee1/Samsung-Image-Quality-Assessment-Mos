...
import pandas as pd
class clipmlp_captionModule(LightningModule):
    ...
    def forward(self, x: torch.Tensor, caption) -> torch.Tensor:
        """Perform a forward pass through the model `self.net`.

        :param x: A tensor of images.
        :return: A tensor of logits.
        """
        return self.net(x, caption)
    ...
    def model_step(
        ...
        """
        img, score, caption = batch[0]['d_img_org'], batch[0]['score'].squeeze(), batch[1]
        pred = self.forward(img, caption)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        # update and log metrics
        self.val_loss(loss)
        self.log("val/loss", self.val_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
