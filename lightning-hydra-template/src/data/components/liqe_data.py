...
class liqe_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        patches = patches[sel, ...]
        scene = self.data.scene[index]
        dists = self.data.dists[index]

        if self.test:
            sample = {"I": patches, "img_name": img_name}
        else:
            sample = {"I": patches, "mos": float(mos), "scene": scene, "dists": dists}
...
class maniqa_crop_dataset(Dataset):
    def __init__(
        self,
        csv_file,
        preprocess,
    ...
    def __getitem__(self, index):
        ...
        if self.test:
            ...
            sample = {"I": patches, "score": float(score)}
...
class AdaptiveResize_448(object):
    """Resize the input PIL Image to the given size adaptively.
    ...
    def __call__(self, img):
        ...
        if h < 448 or w < 448:
            return transforms.Resize((448, 448), self.interpolation)(img)
        elif h < self.size or w < self.size:
            return img
        else:
            return transforms.Resize(self.size, self.interpolation)(img)
...
def _preprocess2_448():
    return Compose(
        [
            _convert_image_to_rgb,
            AdaptiveResize_448(768),
            ToTensor(),
            Normalize(
                (0.48145466, 0.4578275, 0.40821073),
                (0.26862954, 0.26130258, 0.27577711),
...
class Multi_Fidelity_Loss(torch.nn.Module):
    ...
    def forward(self, p, g):
        ...
        for i in range(p.size(1)):
            ...
            p_i = p_i.view(-1, 1)
            loss_i = 1 - (
                torch.sqrt(p_i * g_i + esp) + torch.sqrt((1 - p_i) * (1 - g_i) + esp)
            )
            loss = loss + loss_i
        loss = loss / p.size(1)
